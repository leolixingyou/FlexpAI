...
def lane_line_mask(ll):
    ll_predict = ll[:, :, 12:372, :]  # 영역 조정 필요
    ll_seg_mask = torch.nn.functional.interpolate(ll_predict, scale_factor=2, mode='bilinear')
    ll_seg_mask = torch.round(ll_seg_mask).squeeze(1)
    ll_seg_mask = ll_seg_mask.int().squeeze().cpu().numpy()
    ll_seg_mask = (ll_seg_mask * 255).astype(np.uint8)

    # 추가적인 후처리: 모폴로지 연산으로 노이즈 제거
...
def thin_lane_line_mask(ll_seg_mask):
    kernel = np.ones((5, 5), np.uint8)
    thin_mask = cv2.erode(ll_seg_mask, kernel, iterations=1)

    merged_mask = merge_lane_lines(thin_mask)

    return merged_mask

def merge_lane_lines(mask):
    kernel = np.ones((5, 5), np.uint8)
    dilated_mask = cv2.dilate(mask, kernel, iterations=1)
    eroded_mask = cv2.erode(dilated_mask, kernel, iterations=1)
    return eroded_mask

# --- 슬라이딩 윈도우 방식 차선 검출 함수 ---
